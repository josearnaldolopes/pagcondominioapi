...
switch ($metodo) {
  case "POST":
    ...
    $stmt = $conexao->prepare('INSERT INTO condominio (chave,cep,logradouro,numero,bairro,cidade,estado,condominio,telefone,documento,banco,agencia,conta) VALUES(:chave,:cep,:logradouro,:numero,:bairro,:cidade,:estado,:condominio,:telefone,:documento,:banco,:agencia,:conta)');
    $stmt->execute(array(
      ':chave' => $chave,
      ':cep' => limpanumero($body['cep']),
      ...
      header($http." 201");
      echo json_encode(array('sucesso' => true, 'mensagem' => 'Condominio cadastrado', 'chave' =>  $chave, "consulta" => "https://api.pagcondominio.com/erp/consulta/boleto/$chave"));
  ...
  case "DELETE":
    ...
    if ($acao == "apagar" && $chave) {
      ...
      $sql = $conexao->prepare("DELETE FROM condominio WHERE condominio.id = '$chave'");
      $sql->execute();
      $numero = $sql->rowCount();
      if ($numero) {
        header($http." 200");
  ...
  case "GET":
    ...
    if ($acao == "ver" && $chave) {
      ...
      $sql = $conexao->prepare("SELECT id,condominio,cep,logradouro,numero,bairro,cidade,estado,chave FROM condominio WHERE id = ".$chave."");
      $sql->execute();
      ...
      // $sql = $conexao->prepare("SELECT id,condominio,cep,logradouro,numero,bairro,cidade,estado,chave FROM condominio WHERE erp = ".$registro["id"]."");
      $sql = $conexao->prepare("SELECT id,condominio,cep,logradouro,numero,bairro,cidade,estado FROM condominio");
      $sql->execute();
      $resultado = $sql->fetchAll(PDO::FETCH_ASSOC);
  ...
  case "PUT":
    if ($acao == "alterar") {
      ...
      $conexao->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);  
      // $stmt = $conexao->prepare('UPDATE condominio SET condominio = :condominio, cep = :cep, logradouro = :logradouro, numero = :numero, bairro = :bairro, cidade = :cidade, estado = :estado, telefone = :telefone, documento = :documento, banco = :banco, agencia = :agencia, conta = :conta WHERE chave = :chave');
      $stmt = $conexao->prepare('UPDATE condominio SET condominio = :condominio, cep = :cep, logradouro = :logradouro, numero = :numero, bairro = :bairro, cidade = :cidade, estado = :estado, telefone = :telefone, documento = :documento, banco = :banco, agencia = :agencia, conta = :conta WHERE id = :id');
      $stmt->execute(array(
        ':id' => $body['id'],
        ':condominio' => $body['condominio'],
        ':cep' => limpanumero($body['cep']),
      ...
      $numero = $stmt->rowCount();
        if ($numero) {
          header($http." 200");
          echo json_encode(array('sucesso' => true, 'mensagem' => 'Condominio Alterado', 'data' =>  $data));
        } else {
          header($http." 400");
