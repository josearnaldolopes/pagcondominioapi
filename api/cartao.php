...
switch ($metodo) {
  case "POST":
    ...
    if (empty($acao)) {
      ...
      // die(json_encode(array('status' => 'Debugando', 'data' => $data, 'metodo' => $metodo, 'acao' => $acao, 'Id[chave]' => $chave, 'body' => $body)));
          ...
          $stmt = $conexao->prepare('INSERT INTO cartao (token,idcondomino,documento,numero,digitos,nome,bandeira,cvv,validade) VALUES(:token,:idcondomino,:documento,:numero,:digitos,:nome,:bandeira,:cvv,:validade)');
          $stmt->execute(array(
            ':token' => $token,
  ...
  case "DELETE":
    ...
    if ($acao == "apagar") {
      ...
      $sql = $conexao->prepare("DELETE FROM cartao WHERE cartao.id = '".$chave."'");
      $sql->execute();
      $numero = $sql->rowCount();
      if ($numero) {
        header($http." 200");
  ...
  case "GET":
    ...
    if ($acao == "ver") {
      ...
      $sql = $conexao->prepare("SELECT cartao.id,cartao.idcondomino,cartao.token,cartao.nome,cartao.bandeira,cartao.digitos,cartao.validade FROM cartao WHERE cartao.id = '$chave'");
      $sql->execute();
      ...
      $sql = $conexao->prepare("SELECT cartao.id,cartao.idcondomino,cartao.token,cartao.nome,cartao.bandeira,cartao.digitos,cartao.validade FROM cartao");
      $sql->execute();
